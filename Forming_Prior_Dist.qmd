...
test <- create_prior(ext = c("atp_matches_2022.csv",
                                 "atp_matches_2023.csv"),
...
# so, probability alcaraz wins a point on serve
1 - expit(-0.5603895)
...
djokovic_serving <- djokovic_serving |> 
  mutate(djok_wserv_prob = djok_prob_store)
```
...
# add probabilities to the data frame
alcaraz_serving <- alcaraz_serving |> 
  mutate(alc_wserv_prob = alc_prob_store)
```
...
```{r}
# Manually insert a probability for Alcaraz winning a point on serve in the first row
combined_df[1, "alc_wserv_prob"] <- expit(0.5603895)

# fill in the missing probabilities with the next known probability
combined_filled <- combined_df |>
  fill(djok_wserv_prob, alc_wserv_prob, .direction = "down") |>
  print(n = 30)
# observe probabilities now
combined_filled |> select(djok_wserv_prob, alc_wserv_prob) |>
  print(n = 20)
```
...
djok_serving_f <- tibble(
  ...
  returner.prob = djok_serving$alc_wret_prob
)
...
alc_serving_f <- tibble(
  ...
  returner.prob = alc_serving$djok_wret_prob
)
...
match_data <- data.frame(
  player1 = paste0("`", player1, "`"),
  player2 = paste0("`", player2, "`"),
  p1_server = c(1, 0),
  p2_server = c(0, 1)
)
